		...
		<div class="bg-white border-b w-full">
			<div class="flex flex-col px-6 py-4 space-y-1 cursor-pointer hover:bg-gray-100 w-full"
				...
				<div class="flex flex-row justify-start items-center">
					...
					<Span_btn_article_read cnt_read={props.data.cnt_read.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} />
					<Span_btn_article_comment cnt_comment={props.data.cnt_comment.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} />
				</div>
			</div>
		</div>
	)
