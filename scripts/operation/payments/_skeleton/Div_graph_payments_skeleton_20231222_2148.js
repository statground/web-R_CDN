// 결제 추이 그래프 Skeleton
function Div_graph_payments_skeleton() {
	return (
		<div class="p-4 bg-white rounded-lg md:p-8 text-center animate-pulse" id="div_graph_payments_container">
				...
				<ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="tab_list_graph">
					<li class="mr-2">
						<button class={class_tab_enabled} id="tab_graph_monthly" type="button" 
								onClick={() => get_graph_payments('monthly')}>
							월
						</button>
					</li>
					<li class="mr-2">
						<button class={class_tab_disabled} id="tab_graph_yearly" type="button"
								onClick={() => get_graph_payments('yearly')}>
							년
						</button>
					</li>
				</ul>
