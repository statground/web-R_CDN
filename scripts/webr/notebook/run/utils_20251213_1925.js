...
// Share 토글 API
// =========================
window.apiToggleNotebookShare = async function apiToggleNotebookShare(notebook_uuid, shareOn) {
  const payload = { notebook_id: notebook_uuid, share: shareOn ? 1 : 0 };
  const res = await fetch(NOTEBOOK_API.toggle_share, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) throw new Error("toggle_share_http_" + res.status);
