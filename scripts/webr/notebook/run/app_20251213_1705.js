...
function Notebook() {
  ...
  // =========================
  // View Mode (/notebook/view/<uuid_share>/)
  // =========================
  const isViewMode =
    !!window.WEBR_VIEW_MODE || /\/notebook\/view\//.test(location.pathname || "");
  ...
  useEffect(() => {
    ...
    // View mode에서는 WebR 런타임을 로드하지 않습니다 (읽기 전용 렌더)
    if (isViewMode) {
...
async function handleToggleShare(nextOn) {
  ...
  if (!window.confirm(nextOn ? "이 노트북을 공유(공개)할까요?" : "공유를 해제할까요?")) return;
  ...
  function applyShareJson() { /* deprecated: JSON share */ }
  ...
  async function loadNotebookFromDB(nbid, { setStatusText = true } = {}) {
    ...
    try {
      ...
      // Share 상태 (서버 item 기준)
      try {
        setShareEnabled(!!(res.item && res.item.share));
        setShareUUID((res.item && res.item.uuid_share) ? res.item.uuid_share : "");
      } catch (e) {}

      // 편집 가능 여부 (view 모드면 무조건 false)
      try {
        setCanEdit(!isViewMode && !!res.can_edit);
      } catch (e) {
        setCanEdit(!isViewMode);
      }
    ...
    <div className={rootClass}>
      ...
      <header className={headerClass}>
        ...
        <div className="flex items-center gap-2 text-[12px]">
          {canEdit && (<button type="button" onClick={handleNewNotebook} className="inline-flex items-center gap-1 rounded-full border border-slate-300 px-3 py-1 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
            <span className="font-medium">New</span>
          </button>)}

          {canEdit && (<button type="button" onClick={handleSaveNotebook} className="inline-flex items-center gap-1 rounded-full border border-slate-300 px-3 py-1 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
            <span className="font-medium">Save</span>
          </button>)}

          {canEdit && (<button type="button" onClick={handleReloadFromDB} className="inline-flex items-center gap-1 rounded-full border border-slate-300 px-3 py-1 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              <section key={cell.id} className={cellClass + (activeCellId === cell.id ? " ring-2 ring-orange-400/70" : "")} onClick={() => setActiveCellId(cell.id)}>
                <div className={cellHeaderClass}>
                  ...
                  <div className="flex items-center gap-1 text-[11px]">
                    ...
                    {canEdit && (<button type="button" onClick={() => moveCell(cell.id, "up")} className="inline-flex items-center rounded-full px-1.5 py-0.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                      ↑
                    </button>)}
                    {canEdit && (<button type="button" onClick={() => moveCell(cell.id, "down")} className="inline-flex items-center rounded-full px-1.5 py-0.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                      ↓
                    </button>)}
                    {canEdit && (<button type="button" onClick={() => deleteCell(cell.id)} className="inline-flex items-center rounded-full px-2 py-0.5 text-red-400 hover:bg-red-50 dark:hover:bg-slate-800">
                      ✕
                    </button>)}
                  </div>
                </div>
                  ...
                  {cell.mode === "markdown" ? (
                    cell.mdPreview ? (
                      ...
                      <div>
                        ...
                        <RCodeEditor value={cell.source} onChange={(v) => setCells((prev) => prev.map((c) => (c.id === cell.id ? { ...c, source: v } : c)))} darkMode={darkMode} cellId={cell.id} mode="markdown" onFocus={() => setActiveCellId(cell.id)} readOnly={!canEdit} />
                      </div>
                    )
                  ) : (
                    // R Code 모드
                    <div>
                      ...
                      {cell.showCode !== false && <RCodeEditor value={cell.source} onChange={(v) => setCells((prev) => prev.map((c) => (c.id === cell.id ? { ...c, source: v } : c)))} darkMode={darkMode} cellId={cell.id} mode="r" onFocus={() => setActiveCellId(cell.id)} readOnly={!canEdit} />}
                    </div>
                  )}
        ...
        {/* 사이드바 */}
        {canEdit && (<aside className={(darkMode ? "bg-slate-950 border-l border-slate-800" : "bg-slate-50 border-l border-slate-200") + " w-72 flex-shrink-0 overflow-y-auto px-3 py-4 text-[12px]"}>
          {/* Package Manager */}
    ...
    <div className={"w-full max-w-xl rounded-2xl border p-4 text-sm " + (darkMode ? "border-slate-700 bg-slate-950 text-slate-100" : "border-slate-200 bg-white text-slate-800")}>
      ...
      {isViewMode ? (
        <div className="text-[12px] text-slate-400">
          이 페이지는 공유 보기(View) 모드입니다.
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between rounded-xl border px-3 py-3 text-[12px] dark:border-slate-700">
            <div className="flex flex-col">
              <div className="font-semibold">Public Link</div>
              <div className="text-[11px] text-slate-400">공유를 켜면 누구나 링크로 볼 수 있습니다. (읽기 전용)</div>
            </div>
            ...
            <button
              ...
              className={
                "rounded-full px-4 py-1.5 text-[12px] font-semibold text-white " +
                (shareEnabled ? "bg-emerald-600 hover:bg-emerald-700" : "bg-slate-700 hover:bg-slate-800") +
                (busy || !canEdit ? " opacity-60 cursor-not-allowed" : "")
              }
            >
              {shareEnabled ? "ON" : "OFF"}
            </button>
          ...
          {shareEnabled && shareUUID ? (
            <div className="mt-3 rounded-xl border px-3 py-3 text-[12px] dark:border-slate-700">
              <div className="mb-1 text-[11px] text-slate-400">Share URL</div>
              <div className="flex items-center gap-2">
                <input
                  readOnly
                  value={`${location.origin}/webr/notebook/view/${shareUUID}/`}
                  className={"flex-1 rounded-lg border px-2 py-1 text-[11px] " + (darkMode ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-200 bg-slate-50 text-slate-800")}
                />
                <button
                  type="button"
                  onClick={() => navigator.clipboard.writeText(`${location.origin}/webr/notebook/view/${shareUUID}/`).then(() => alert("Copied"))}
