...
function Notebook() {
  ...
  function restoreNotebookFromDbItem(item) {
    ...
    return {
      ...
      darkMode: (typeof meta.darkMode !== "undefined") ? !!meta.darkMode : null,
      dataFiles: Array.isArray(dataMgr) ? dataMgr : [],
  ...
  async function loadNotebookFromDB(nbid, { setStatusText = true } = {}) {
    ...
    try {
      ...
      // 다양한 응답 포맷을 흡수 (item / data / notebook / result / items[0] 등)
      const okFlag = (res && (res.ok === true || res.ok === 1 || res.ok === "1" || res.ok === "true"));
      const item =
        (res && (res.item || res.data || res.notebook || res.result || res.row)) ||
        (res && Array.isArray(res.items) && res.items.length ? res.items[0] : null);

      if (!okFlag || !item) {
        console.warn("loadNotebookFromDB: unexpected response", res);
        if (setStatusText) setStatus("Load failed");
        return false;
      }

      const restored = restoreNotebookFromDbItem(item);
