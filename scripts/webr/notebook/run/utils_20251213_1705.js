...
/**
 * Notebook Share Toggle (DB)
 * backend expects: notebook_id, share(0/1)
 */
window.apiToggleNotebookShare = async function apiToggleNotebookShare(notebook_id_like, shareOn) {
  const notebook_id =
    getNotebookUUIDFromPayload({ notebook_id: notebook_id_like }) ||
    notebook_id_like ||
    guessNotebookUUIDFromLocation();

  const body = JSON.stringify({ notebook_id, share: shareOn ? 1 : 0 });
