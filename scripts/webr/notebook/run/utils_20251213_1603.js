...
/**
 * ArrayBuffer -> base64 (for DB persistence)
 */
...
/**
 * R 변수명으로 안전하게 변환
 * - 알파벳/숫자/_ 만 허용, 숫자로 시작하면 앞에 df_ 붙임
 */
function toSafeRVarName(str, fallback = "df") {
  const s = (str || "").toLowerCase().replace(/[^a-z0-9_]+/g, "_").replace(/^_+|_+$/g, "");
  if (!s) return fallback;
  if (/^[0-9]/.test(s)) return `${fallback}_${s}`;
  return s;
}

/**
 * 동일 파일명 업로드 대비: 이미 존재하면 (2), (3) ... suffix를 붙여 유니크하게 만든다.
 */
function makeUniqueFileName(existingNames, desiredName) {
  ...
  let n = 2;
  while (names.has(`${base} (${n})${ext}`)) n += 1;
  return `${base} (${n})${ext}`;
}
