...
function Notebook() {
  ...
  // =========================
  // DB Load: 페이지 진입 시 notebook data 로드 (split columns)
  // =========================
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (restored && restored.cells) {
          setCells(restored.cells);
          if (restored.cells.length > 0) setActiveCellId(restored.activeCellId || restored.cells[0].id);
        }
        if (typeof restored.darkMode === "boolean") setDarkMode(restored.darkMode);

        if (Array.isArray(restored.dataFiles)) setDataFiles(restored.dataFiles);
        if (Array.isArray(restored.installedPackages)) {
  ...
  // =========================
  // ✅ Split-Column DB Serialize/Restore Helpers
  // - data_markdown: markdown cell sources
  // - data_rcode: r cell sources
  // - data_rcode_result: r outputs (text/image)
  // - data_data: Data Manager files list
  // - data_rpackage: installed packages list
  // - data_meta: everything else (order, toggles, darkMode, activeCellId, version, etc.)
  // =========================
  function safeJsonParse(v, fallback) {
    try {
      ...
      // TiDB JSON may already be object
      return v;
    } catch (e) {
      return fallback;
    }
  ...
  function restoreNotebookFromDbItem(item) {
    // title은 별도로 setNotebookTitle에서 처리
    const mdArr   = safeJsonParse(item && item.data_markdown, []);
    const rArr    = safeJsonParse(item && item.data_rcode, []);
    const rrArr   = safeJsonParse(item && item.data_rcode_result, []);
    const dataArr = safeJsonParse(item && item.data_data, []);
    const pkgArr  = safeJsonParse(item && item.data_rpackage, []);
    const metaObj = safeJsonParse(item && item.data_meta, {});

    const order = Array.isArray(metaObj.cell_order) ? metaObj.cell_order : null;
    const mdMap = new Map((Array.isArray(mdArr) ? mdArr : []).map((x) => [Number(x.id), x]));
    const rMap  = new Map((Array.isArray(rArr) ? rArr : []).map((x) => [Number(x.id), x]));
    const rrMap = new Map((Array.isArray(rrArr) ? rrArr : []).map((x) => [Number(x.id), x]));

    // cell ids union (order 우선)
    const idSet = new Set();
    (Array.isArray(mdArr) ? mdArr : []).forEach((x) => idSet.add(Number(x.id)));
    (Array.isArray(rArr) ? rArr : []).forEach((x) => idSet.add(Number(x.id)));
    if (order && order.length) order.forEach((id) => idSet.add(Number(id)));

    let ids = Array.from(idSet).filter((x) => Number.isFinite(x) && x > 0);
    if (order && order.length) {
      const orderNums = order.map((x) => Number(x)).filter((x) => Number.isFinite(x) && x > 0);
      // order에 있는 걸 먼저, 나머지는 뒤에
      const rest = ids.filter((x) => !orderNums.includes(x)).sort((a, b) => a - b);
      ids = [...orderNums, ...rest];
    } else {
      ids.sort((a, b) => a - b);
    }
    ...
    if (!ids.length) {
      ...
      return {
        ...
        darkMode: null,
        dataFiles: [],
        installedPackages: [...CORE_PACKAGES],
      };
    }

    const cellsRestored = ids.map((id) => {
      // meta per cell
      const per = (metaObj && metaObj.cells && metaObj.cells[String(id)]) ? metaObj.cells[String(id)] : {};
      const showCode = per.showCode !== false;
      const showOutput = per.showOutput !== false;
      const mdPreview = per.mdPreview !== false; // markdown 기본 true

      // markdown cell?
      if (mdMap.has(id) && (!rMap.has(id) || per.mode === "markdown")) {
        const src = (mdMap.get(id).source || "");
        return {
          id,
          mode: "markdown",
          source: src,
          output: { type: "markdown", html: renderMarkdown(src) },
          mdPreview: true,
          showCode,
          showOutput,
        };
      ...
      // r cell default
      const src = rMap.has(id) ? (rMap.get(id).source || "") : "";
      let output = null;
      const savedOut = rrMap.has(id) ? rrMap.get(id).output : null;
      if (savedOut && savedOut.type === "text") {
        output = { type: "text", text: savedOut.text || "" };
      } else if (savedOut && savedOut.type === "image") {
        output = {
          type: "image",
          src: savedOut.src,
          inlineSize: savedOut.inlineSize || "small",
          toggleSize: () => {},
        };
      }
    ...
    // image toggleSize 연결
    const finalCells = cellsRestored.map((c) => {
      if (c.output && c.output.type === "image") {
        c.output.toggleSize = () => {
          setCells((prev) =>
            prev.map((p) => {
              if (p.id !== c.id) return p;
              const sz = (p.output && p.output.inlineSize) === "large" ? "small" : "large";
    ...
    // next cell id seq sync
    try {
      const maxId = Math.max(...finalCells.map((x) => Number(x.id) || 0), 2);
      if (typeof __cell_id_seq !== "undefined") __cell_id_seq = maxId;
    } catch (e) {}

    return {
      cells: finalCells,
      activeCellId: (metaObj && Number(metaObj.activeCellId)) || finalCells[0].id,
      darkMode: (typeof metaObj.darkMode === "boolean") ? metaObj.darkMode : null,
      dataFiles: Array.isArray(dataArr) ? dataArr : [],
      installedPackages: Array.isArray(pkgArr) ? pkgArr : [],
    };
  }
  ...
  function buildDbPartsFromNotebookState() {
    ...
    const data_rcode_result = cells
      ...
      .map((c) => {
        ...
        if (c.output && c.output.type === "text") {
          out = { type: "text", text: c.output.text || "" };
        } else if (c.output && c.output.type === "image") {
          out = { type: "image", src: c.output.src, inlineSize: c.output.inlineSize || "small" };
        }
        return { id: c.id, output: out };
      });
    ...
    const meta = {
      ...
      cells: cells.reduce((acc, c) => {
        acc[String(c.id)] = {
          ...
          mdPreview: c.mode === "markdown" ? (c.mdPreview !== false) : false,
          mode: c.mode,
        };
        return acc;
      }, {}),
    ...
    // data manager / package manager
    const data_data = Array.isArray(dataFiles) ? dataFiles : [];
    const data_rpackage = Array.isArray(installedPackages) ? installedPackages : [];
...
// =========================
  ...
  async function handleSaveNotebook() {
    ...
    try {
      ...
      const res = await apiSaveNotebook(nbid, notebookTitle, parts);
    ...
    try {
      setBusy(true);
      setStatus("Saving...");
      const res = await apiSaveNotebook(nbid, notebookTitle, JSON.stringify(payload));
      if (res && res.ok) {
        setStatus("Saved");
      } else {
        alert((res && res.msg) ? res.msg : "Save failed");
  ...
  async function handleReloadFromDB() {
    ...
    // ✅ 로드 확인 (Yes/No)
    const ok = window.confirm("DB에서 다시 불러올까요? (현재 변경사항은 저장되지 않습니다)");
    if (!ok) return;
