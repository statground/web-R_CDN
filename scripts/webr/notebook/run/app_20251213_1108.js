...
function Notebook() {
  ...
  // =========================
  // ✅ Auth (실제 로그인/로그아웃)
  // - 기존에는 로그인 모달이 "데모용"이어서 실제 API 호출을 하지 않았음
  // - (중복 UI의 원인이었던) 별도 Fixed Auth Widget을 제거하더라도,
  //   여기 React 헤더/모달에서 실제 인증을 수행하도록 이식한다.
  // =========================
  ...
  /**
   * 페이지 진입 시 1회 + 포커스 복귀 + 주기적 폴링으로 인증 상태를 동기화
   */
  useEffect(() => {
    refreshAuth();
