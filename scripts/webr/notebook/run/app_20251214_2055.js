...
// =========================
// Sidebar (Package/Data) responsive toggle
// - Mobile: default CLOSED (drawer)
// - Desktop: default OPEN (fixed)
// =========================
...
useEffect(() => {
  try {
    ...
    // fallback: treat as desktop
    setIsDesktop(true);
    setSidebarOpen(true);
  }
}, []);

// Lock body scroll when mobile drawer is open (better UX)
useEffect(() => {
  try {
    if (!isDesktop) {
    ...
    <div className={rootClass}>
      ...
      <header className={headerClass}>
        <div className="flex items-center gap-3">
          ...
          {/* ✅ Running... 영역은 항상 같은 공간을 차지하게 해서 클릭/레이아웃 시프트를 방지 */}
<span
  className={
    "flex items-center gap-1 text-[11px] text-amber-400 transition-opacity " +
...
<aside
  className={
    ...
    (isDesktop
      ? "relative z-0"
      : "fixed z-40 top-14 right-0 bottom-0 shadow-2xl transition-transform duration-200 " +
        (sidebarOpen ? "translate-x-0" : "translate-x-full"))
  }
>
  {/* (Mobile only) header + close */}
  {!isDesktop && (
    <div className="mb-3 flex items-center justify-between">
