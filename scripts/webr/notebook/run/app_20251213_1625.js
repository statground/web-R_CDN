...
function Notebook() {
  ...
  async function handleDataUpload(e) {
    ...
    // 기존 파일명/변수명 충돌 방지
    const existingNames = (dataFiles || []).map((f) => f && f.name).filter(Boolean);
    const uniqueName = makeUniqueFilename(file.name, existingNames);

    const ext = getDataFileExtension(uniqueName);
    const existingVars = (dataFiles || []).map((f) => f && f.rVar).filter(Boolean);
    ...
    // xlsx는 readxl 필요: 설치목록/런타임 설치까지 자동 처리
    const needPkgs = [];
    ...
    try {
      ...
      for (const pkg of needPkgs) {
        ...
        try {
          ...
          console.warn("Auto install failed:", pkg, e2);
        }
      }
  ...
  /**
   * ✅ 노트북 로드 후 런타임(WebR) 상태 복원
   * - 패키지: data_rpackage 기반으로 installPackages 재실행
   * - 파일: data_data의 contentBase64를 WebR FS로 재주입
   */
  async function restoreRuntimeState(restored) {
    ...
    const wantPkgs = pkgs.filter((p) => p && !CORE_PACKAGES.includes(p));

    if (wantPkgs.length) {
      setStatus("Restoring packages...");
      for (const pkg of wantPkgs) {
        try {
          await webrInstance.installPackages([pkg]);
        } catch (e) {
          console.warn("restore package failed:", pkg, e);
        }
      }
    }
    ...
    if (files.length) {
      ...
      for (const f of files) {
        try {
          ...
          if (!f.contentBase64) continue; // 과거 데이터(바이너리 미저장)면 스킵
