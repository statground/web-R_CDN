...
function Notebook() {
  ...
  /**
   * ✅ Dark Mode 토글 처리
   * - Tailwind의 dark: 변형이 동작하도록 documentElement에 'dark' 클래스를 붙였다/뗀다.
   * - CodeMirror는 Tailwind 영향을 직접 받지 않으므로, 테마 CSS를 로드하고
   *   모든 CodeMirror 인스턴스에 theme 옵션을 적용 + refresh 해준다.
   */
  useEffect(() => {
    // Tailwind dark: 활성화
    try {
    ...
    // CodeMirror 테마 적용 + 하이라이트/레이아웃 리프레시
    // - '어느 순간 하이라이트가 안 보이는' 문제는 refresh 누락/테마 불일치에서 자주 발생
    setTimeout(() => {
      try {
        document.querySelectorAll(".CodeMirror").forEach((el) => {
          ...
          // 모드가 비어있는 경우(예: 옵션 누락) 기본값 보정
          // - R 셀은 r, Markdown 셀은 markdown으로 설정되어야 하이라이트가 동작함
          try {
            const mode = cm.getOption("mode");
            if (!mode) cm.setOption("mode", "r");
          } catch (e) {}
  ...
  /**
   * ✅ CodeMirror 상태 보정
   * - 셀이 추가/삭제/모드 변경될 때 CodeMirror가 간헐적으로 하이라이트/레이아웃이 깨지는 경우가 있어
   *   전체 인스턴스에 대해 refresh + theme 적용을 한 번 더 수행한다.
   */
  useEffect(() => {
    setTimeout(() => {
      try {
  ...
  async function runRCell(id, insertBelow = false) {
    ...
    try {
          ...
          toggleSize: () => {
            setCells((prev) =>
              prev.map((c) => {
                if (c.id !== id) return c;
                const currentSize =
                  (c.output && c.output.inlineSize) === "large"
                    ? "small"
                    : "large";
                return {
                  ...c,
                  output: { ...c.output, inlineSize: currentSize },
                };
              })
            );
          },
      ...
      setCells((prev) =>
        prev.map((c) =>
          c.id === id
            ? { ...c, output: { type: "text", text: String(e.message || e) } }
            : c
        )
      );
  ...
  function runMarkdownCell(id) {
    ...
    setCells((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, output: newOutput, mdPreview: true } : c
      )
    );
  ...
  useEffect(() => {
    function handleKeyDown(e) {
      ...
      const isRunCombo =
        key === "Enter" && !e.altKey && (e.shiftKey || e.ctrlKey || e.metaKey);
      const isHelpCombo =
        (key === "h" || key === "H") &&
        (e.altKey || (e.ctrlKey && e.shiftKey) || (e.metaKey && e.shiftKey));
    ...
    reader.onload = () => {
      try {
        ...
        const loadedCells = parsed.cells.map((cell) => {
          ...
          if (cell.output) {
            if (cell.output.type === "text")
              restoredOutput = { type: "text", text: cell.output.text };
  ...
  const cellHeaderClass =
    ...
    (darkMode ? "border-stone-800/80 bg-stone-950/90" : "border-slate-200 bg-slate-50");
    ...
    <div className={rootClass}>
      ...
      <header className={headerClass}>
        <div className="flex items-center gap-3">
          ...
          <span
            className={
              "ml-4 rounded-full px-2 py-0.5 text-[11px] " +
        ...
        <main className="flex-1 overflow-y-auto px-6 py-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <input
                type="text"
                defaultValue="Untitled Web-R Notebook"
                className={
                  ...
                  <div className="flex items-center gap-2 text-[12px]">
                    ...
                    <div className="inline-flex items-center rounded-full bg-slate-900/5 p-0.5 dark:bg-slate-800/60">
                      ...
                      <button
                        ...
                        className={
                          "px-2 py-0.5 text-[11px] rounded-full " + (cell.mode === "markdown" ? "bg-orange-500 text-white" : "text-slate-500")
                        }
                      >
                        Markdown (LaTeX)
                      </button>
          ...
          <section className="mb-4">
            ...
            <div className="mb-2 flex gap-1">
              <input
                ...
                className={"flex-1 rounded-lg border px-2 py-1 text-xs " + (darkMode ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-200 bg-white text-slate-900")}
              />
